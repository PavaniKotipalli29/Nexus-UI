...
export const Wizard: React.FC<WizardProps> = ({
  ...
  const getVariantStyles = () => {
    switch (variant) {
      case 'filled':
        return 'bg-neutral-100/50 dark:bg-neutral-900 border border-neutral-200/60 dark:border-neutral-800 p-8 rounded-3xl shadow-sm';
      case 'outline':
        return 'border-2 border-neutral-100 dark:border-neutral-800 p-8 rounded-3xl';
      case 'glass':
        return 'backdrop-blur-2xl bg-white/60 dark:bg-neutral-950/60 border border-white/40 dark:border-white/10 p-8 rounded-3xl shadow-2xl ring-1 ring-black/5';
      case 'gradient':
        return `bg-gradient-to-br from-${brandColor}-50/50 to-neutral-50 dark:from-${brandColor}-950/20 dark:to-neutral-950/20 p-8 rounded-3xl border border-${brandColor}-200/50 dark:border-${brandColor}-800/50 shadow-inner`;
      case 'minimal':
        return 'p-0 bg-transparent border-none';
      default:
        return 'py-4';
    }
  };
        ...
        {steps.map((step, index) => {
          ...
          return (
            <React.Fragment key={step.id || index}>
              <div 
                className={`flex ${isHorizontal ? 'flex-col items-center flex-1 text-center px-4' : 'flex-row items-center gap-6'} group cursor-pointer relative z-10`}
              ...
              >
                ...
                <div className={`${isHorizontal ? 'mt-4' : ''} flex flex-col ${isHorizontal ? 'items-center' : 'items-start'}`}>
                  <h4 className={`${sizes[size].text} ${styles.text} transition-colors duration-300`}>
                    {step.title}
                  </h4>
                  {step.description && (
                    <p className={`text-xs mt-1 text-neutral-500 dark:text-neutral-400 max-w-[200px] line-clamp-2 ${isHorizontal ? 'text-center' : 'text-left'}`}>
                      {step.description}
                    </p>
                  )}
                </div>
                ...
                {!isLast && !isHorizontal && (
                  ...
                  >
                    <div 
                      className={`w-full h-full transition-all duration-700 ease-in-out ${styles.line}`}
              ...
              {!isLast && isHorizontal && (
                <div className={`flex-1 ${sizes[size].line} mt-[${size === 'sm' ? '16px' : size === 'lg' ? '24px' : '20px'}] -mx-[50%] z-0 border-none relative overflow-hidden bg-neutral-100 dark:bg-neutral-800 rounded-full`}>
                  <div 
                    className={`h-full transition-all duration-700 ease-in-out ${styles.line} shadow-[0_0_8px_rgba(var(--color-primary-rgb),0.5)]`} 
